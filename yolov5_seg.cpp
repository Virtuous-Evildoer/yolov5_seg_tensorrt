...
//        std::cout << "msk Over " << std::endl;
//        
//        draw_mask_bbox(img, res, masks, labels_map);
//        std::cout << "draw Over " << std::endl;
...
extern "C" __declspec(dllexport) void* Init(char* model_path)
{
	...
	trt->engine = nullptr;
	trt->context = nullptr;
	deserialize_engine(engine_name, &trt->runtime, &trt->engine, &trt->context);
	CUDA_CHECK(cudaStreamCreate(&trt->stream));
	cuda_preprocess_init(kMaxInputImageSize);

	trt->cpu_output_buffer1 = nullptr;
	trt->cpu_output_buffer2 = nullptr;
	prepare_buffers(trt->engine, &trt->gpu_buffers[0], &trt->gpu_buffers[1], &trt->gpu_buffers[2], &trt->cpu_output_buffer1, &trt->cpu_output_buffer2);

	return trt;
}

extern "C" __declspec(dllexport) void Detect(Yolov5TRTContext* h, char* image_path, char* labels_filename, float(*res_array)[4], uchar* array)
{
	...
	std::vector<cv::Mat> img_batch;
	img_batch.push_back(img);
	cuda_batch_preprocess(img_batch, trt->gpu_buffers[0], kInputW, kInputH, trt->stream);
	// inference
	infer(*trt->context, trt->stream, (void**)trt->gpu_buffers, trt->cpu_output_buffer1, trt->cpu_output_buffer2, kBatchSize);
	//NMS
	std::vector<std::vector<Detection>> res_batch;
	batch_nms(res_batch, trt->cpu_output_buffer1, img_batch.size(), kOutputSize1, kConfThresh, kNmsThresh);

	auto& res = res_batch[0];

	cv::Mat img1(img_batch[0].size(), img_batch[0].type(), Scalar(255, 255, 255));//全白图
	auto masks = process_mask(&trt->cpu_output_buffer2[0 * kOutputSize2], kOutputSize2, res);
	//draw_mask_bbox(img, res, masks, labels_map);
	//draw
	...
	for (size_t i = 0; i < res.size(); i++) {
		cv::Mat img_mask = scale_mask(masks[i], img1);
		auto color = colors[(int)res[i].class_id % colors.size()];
		auto bgr = cv::Scalar(color & 0xFF, color >> 8 & 0xFF, color >> 16 & 0xFF);

		cv::Rect r = get_rect(img1, res[i].bbox);
		int tr_width = r.x + r.width;
		int tr_height = r.y + r.height;
		if (tr_width > img_mask.cols)
			tr_width = img_mask.cols;
		if (tr_height > img_mask.rows)
			tr_height = img_mask.rows;
		for (int x = r.x; x < tr_width; x++) {
			for (int y = r.y; y < tr_height; y++) {
				float val = img_mask.at<float>(y, x);
				if (val <= 0.5) continue;
				img1.at<cv::Vec3b>(y, x)[0] = img1.at<cv::Vec3b>(y, x)[0] / 2 + bgr[0] / 2;
				img1.at<cv::Vec3b>(y, x)[1] = img1.at<cv::Vec3b>(y, x)[1] / 2 + bgr[1] / 2;
				img1.at<cv::Vec3b>(y, x)[2] = img1.at<cv::Vec3b>(y, x)[2] / 2 + bgr[2] / 2;
			}
		}

		res_array[i][0] = r.x;  
		res_array[i][1] = r.y;
		res_array[i][2] = r.width;
		res_array[i][3] = r.height;
	}
	//cv::imwrite("_123.bmp" , img1);
	cvtColor(img1, img1, CV_RGB2GRAY);
	Mat img2 = img1.clone();
	//cv::imwrite("_123.bmp", img2);
	//array = new uchar[img2.rows * img2.cols];
	/*if (img2.isContinuous())
		array = img2.data;*/
	for (int i = 0; i < img2.rows; i++) {
		for (int j = 0; j < img2.cols; j++) {
			array[i* img2.cols +j] = img2.at<uchar>(i, j);
		}
	}/*
	array[0] = 6;
	array[1] = img2.at<uchar>(5, 2);*/
...
extern "C" __declspec(dllexport) void cuda_free(void* h) {
	...
	CUDA_CHECK(cudaFree(trt->gpu_buffers[0]));
	CUDA_CHECK(cudaFree(trt->gpu_buffers[1]));
	CUDA_CHECK(cudaFree(trt->gpu_buffers[2]));
	delete[] trt->cpu_output_buffer1;
	delete[] trt->cpu_output_buffer2;
	cuda_preprocess_destroy();
	// Destroy the engine
	trt->context->destroy();
	trt->engine->destroy();
...
//int main(int argc, char** argv) {
//
//	char* model_path = "D:\\glass detect\\yolov5_seg_tensorrt\\build\\Release\\best.engine";
//	char* image_path = "D:\\glass detect\\yolov5_seg_tensorrt\\build\\Release\\val\\1.bmp";
//	char* label = "D:\\glass detect\\yolov5_seg_tensorrt\\build\\Release\\133_60.txt";
//	//float(*res)[4] = { 1，2 };
//	Yolov5TRTContext* h = new Yolov5TRTContext();
//	h = Init(model_path);
//	Detect(h, image_path, label);
//	cuda_free(h);
//		
//
//}
